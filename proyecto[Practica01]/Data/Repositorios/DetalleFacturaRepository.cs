...
namespace proyecto_Practica01_.Data.Repositorios
{
    internal class DetalleFacturaRepository : IDetalleFacturaRepository
    {
        ...
        public bool Save(DetalleFactura detalleFactura)
        {
            List<ParameterSP> param = new List<ParameterSP>()
            {
                new("@id_articulo", detalleFactura.Articulo.Id_articulo),
                new("@id_factura", detalleFactura.Factura.NroFactura),
                new("@cantidad", detalleFactura.cantidad),
                new("@precio", detalleFactura.precio),
            };
            return DataHelper.GetInstance().ExecuteSPNonQuery("SP_INSERTAR_DETALLE_FACTURA", param);
        }
    }
}
